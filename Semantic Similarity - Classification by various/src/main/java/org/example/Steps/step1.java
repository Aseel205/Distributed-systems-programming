...
public class step1 {
    ...
    public static class MapperClass extends Mapper<LongWritable, Text, Text, Text> {
        ...
        protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            for (String token : tokens) {
                ...
                String Word = tokenParts[0].toLowerCase(); // The actual feature word
                String depLabel = tokenParts[2]; // Dependency label

                // Apply the stemmer to the feature word
                stemmer = new Stemmer();
                stemmer.add(Word.toCharArray(), Word.length()); // Add each character to the stemmer
                ...
                if (Lexems.contains(Word)) {
                    context.write(new Text("L"), new Text(String.valueOf(totalCount))); // Count all lexemes
                    context.write(new Text("$ " + Word), new Text(String.valueOf(totalCount))); // here we had an issue ...
                } 
    ...
    public static void main(String[] args) throws Exception {
        ...
        job.setCombinerClass(step1.ReducerClass.class);         // combiner
