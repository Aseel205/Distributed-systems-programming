...
public class wordPairSimilaritySmallInput {


    private static final String WORD_RELATEDNESS_FILE = "src/main/resources/inputFiles/word-relatedness.txt";
    private static final String STEP3_OUTPUT_FOLDER = "src/main/resources/OutputFiles-small corupos /step3Outpot";
...
////  Distance functions
    ...
    public static double cosineSimilarity(double[] vec1, double[] vec2) {
        ...
        // Output the dot product and final cosine similarity
        return result;
    }
    ...
    public static void computeSimilarities(List<String[]> wordPairs, Map<String, double[][]> wordVectors) throws IOException {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("src/main/resources/OutputFiles-small corupos /step4Output/similarityScores.txt"))) {
            ...
            for (String[] pair : wordPairs) {
                ...
                String[] methodNames = {
                        "manhattanDistance", "euclideanDistance", "cosineSimilarity",
                        "jaccardSimilarity", "diceSimilarity", "jensenshannonDivergence"
                };

                for (int methodIndex = 0; methodIndex < 6; methodIndex++) {
                    formattedScores.append(methodNames[methodIndex]).append(": ");
                    for (int vectorIndex = 0; vectorIndex < 4; vectorIndex++) {
                        ...
                        if (vectorIndex != 3) { // Add a comma between scores (except for the last one in each method group)
                            formattedScores.append(", ");
                        }
                    }
                    if (methodIndex != 5) { // Add a newline between groups (except for the last group)
                        formattedScores.append("\n");
                    }
                }

                // Write the word pair and the formatted similarity scores to the file
                writer.write(word1 + "\t" + word2 + "\t" + relation + "\n" + formattedScores.toString());
